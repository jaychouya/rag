...
class CodeExtractor:
    ...
    def __init__(self, llm: AbsLLM, max_workers=5, title="agent", max_retry=3, enable_thinking: Optional[bool]=None, call_timeout: Optional[float]=None) -> None:
        self.title = title
        self.llm = llm
        self.max_retry = max_retry
        self.max_workers = max_workers
        # 使用线程信号量进行全局并发控制（支持跨线程和事件循环）
    ...
    async def do(
        ...
        try:
            # 执行实际的LLM调用
            result = await self._do_with_llm(message, system_message, validation_method, codeType, debug, debug_out)
            return result
        finally:
            # 确保信号量被释放
    ...
    async def _do_with_llm(
        ...
        while retry_count < self.max_retry:
            try:
                ...
                _coro = self.llm.chat(message, system_message=system_message, enable_thinking=self.enable_thinking)
                if self.call_timeout:
                    chat_response = await asyncio.wait_for(_coro, timeout=self.call_timeout)
                else:
                    chat_response = await _coro
